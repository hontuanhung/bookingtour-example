...
exports.signup = catchAsync(async (req, res, next) => {
  const validator = validators.validateSignup(req.body);
  if (!validator[0]) {
    return next(validator[1]);
  }
...
exports.login = catchAsync(async (req, res, next) => {
  const error = validators.validateData(req.body, {
  ...
  // const validator = validators.validateLogin(req.body);
  // if (!validator[0]) {
  //   return next(validator[1]);
  // }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  const validator = validators.validateForgotPassword(req.body);
  if (!validator[0]) {
    return next(validator[1]);
  }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  const validator = validators.isPasswordMatched(
    req.body.password,
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  const validator = validators.validateUpdatePassword(req.body);
  if (!validator[0]) {
    return next(validator[1]);
  }
